...
(defun read-fasta (filename)
  ...
  (let ((in (open filename :if-does-not-exist nil))
    ...
    (when in;When in  is  not  nil, thatis, when the file was read correctly.
      ...
      (when (char= (elt fastaname 0) #\>) ;checks the fasta format.
        ...
        (when fastalist;when  fastalist  is  not  nil
          ...
          ;the header name  of  the  protein  and  its   amino acidsequence.
        	 (setq proteins (append proteins (list (list (subseq fastaname 1) fastalist))))
    ...
    ;containing the name and amino     acid sequenceof  the  protein.
    );let
  )
;-------------------------------------------------------------------------------
...
(defun histo-fasta( filename )
  (let  ((multifastalist (read-fasta filename ))
       	 ...
       	 (fastaname)
       	 (histogram nil))
    ;This  dolist   scans the  file'sprotein list read by read-fasta function. 
    (dolist (fastalist multifastalist histogram)
            ...
            ;This  dolist  scans the list of amino acidss definidain  aatargets, and counts the amount oftimes
            ...
            ;Here, with  func  append,  the  blending of the proteins  analyzedis being made,
            ;by creating a unique  list  with  sub-lists,whereeach  sub-list is a  representation
            ;of the protein histogram,  containing the nameof the  protein  and the  frequency  of times each amino acid
...
;;Block functions to read files from a directory
    ...
    (dolist (p pangenoma)
		...
		(if (and similar (first similar) (second similar))
		    (progn
              ...
              ;;Reconfiguring  the  profile-phylo-p variable 
                           ...
                           #|(progn
                              ...
                              (if (< subtracao ppiterlimit)
                                (progn
                                 ...
                                 (do ((x 2 (incf x)))
                                   ((> (/(* x (- x 1)) 2) resto));if (/(* x (- x 1)) 2) is greater than the rest, then for the loop.
...
(defun phylogenetic-profiles-trim (percentage-pp pptolerance trim)
  (let ((ppi (list));;List to   store the interactions created.
                    (ppi-by-score (list));List for storing PPIs per port.
                    (grupos-identicos (list));List for storing protein groups with identical profiles.
                    (grupos-similares (list));List for storing protein groups with identical and similar profiles.
                    (pesos-grupos (list))
                    (posicao-grupo);Variable to store the position of a group g.
              ...
              ((= pptolerance 0)
               (progn
                ...
                (dolist (g grupos-identicos)
                        ...
                        ;;Generating PPis for all protein pair of group g.
                        (dotimes (i (1- (third g)))
               ...
               (progn
                ...
                (dolist (g grupos-similares)
                        ...
                        (dolist (grupo-similar (second g))
                                ;;Calculating the  difference  between profiles.
                                (setf diferenca-perfis (comparar-perfis (first (first g)) (first grupo-similar)))
                                ;;by inserting the new weight generated into the list of PPis by weight.
            ...
            (cond
              ...
              ((> pptolerance 0)
               (progn
                ...
                ;;Ordering the similar groups by size, from smallest to largest.
                (setf grupos-similares (sort grupos-similares #'< :key #'(lambda (x) (third (first x)))))
                ...
                (dolist (g grupos-similares)
                        ...
                        (dolist (grupo-similar (second g))
                                ;Calculating the  difference  between profiles.
                                (setf diferenca-perfis (comparar-perfis (first (first g)) (first grupo-similar)))
                                ;;by inserting the new weight generated to the list of PPis by weight.
                                (push (list (- peso-grupo (* peso-grupo (/ (/ (* diferenca-perfis 100) (length *genomes-files*)) 100))) (list)) ppi-by-score)
              ...
              ((= pptolerance 0)
               (progn
                ...
                (dolist (g grupos-identicos)
                        ...
                        ;;Calculating the weight of group g based on its position, and the position of the last group (most populous).
                        (setf peso-grupo (* (/ dividendo divisor) percentage-pp))
               ...
               (progn
                ...
                (dolist (g grupos-similares)
                        ...
                        (dolist (grupo-similar (second g))
                                ...
                                ;;Sweeping the proteins of a group with similar profile.
                                (dolist (ps (second grupo-similar))
                                        ;;Sweeping proteins   from identical profiles of a group g.
                                        (dolist (pg (second (first g)))
                                                ;;Creating  an Interaction.
                                          					 (push (make-ppi-struct
                                                 						:genea (if (< (second pg) (second ps)) (first pg) (first ps) )
