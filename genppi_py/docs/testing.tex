...
\textbf{Important Note:} Version 0.1.7 exclusively uses the py7zr library to extract multi-volume \texttt{model.7z} files. The dependencies \texttt{py7zr>=1.1.3} (resolving CVE-2026-23879) and \texttt{multivolumefile>=0.2.3} are installed automatically.
...
pip install --upgrade --force-reinstall "py7zr>=1.1.3,<2.0.0" "multivolumefile>=0.2.3,<0.3.0"
        \end{lstlisting}
